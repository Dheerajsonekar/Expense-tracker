...
exports.expensePost = async (req, res) => {
  ...
  try {
    const response = await expense.create({ amount, description, category, userId: req.user.id });
    if (response) {
...
exports.showExpenses = async (req, res)=>{
    ...
    try{
      ...
      const response = await expense.findAll( {where: {userId: req.user.id}, orders:[['createdAt', 'DESC']]});
      if(response){
